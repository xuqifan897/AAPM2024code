...
def examineDose_new():
    ...
    patientFolder = "/data/qifan/projects/FastDoseWorkplace/TCIASelect/013"
...
def examineMask():
    ...
    if True:
        # interpolate imperfect slices
        idx_start = 43
        idx_end = 49
        idx_range = idx_end - idx_start
        template_start = maskArray[idx_start, :, :].astype(float)
        template_end = maskArray[idx_end, :, :].astype(float)
        for i in range(idx_start+1, idx_end):
            slice = (idx_end-i) / idx_range * template_start + (i-idx_start) / idx_range * template_end
            slice = slice > 0.5
            maskArray[i, :, :] = slice
...
def StructInfoGen():
    ...
    if True:
        ...
        expFolder = "/data/qifan/projects/FastDoseWorkplace/TCIASelect/013/FastDose"
