...
def doseMerge():
    """
    This function merges the dose of all fractions
    """
    pattern = "/mnt/shengdata1/qifan/TCIA/Head-Neck-Cetuximab/" \
        "TCIA_Head-Neck_Cetuximab_06-22-2015/Head-Neck_Cetuximab/" \
        "0522c0057/05-01-2000-522-26566/*Dose*/1-1.dcm"
    ...
    for file in matching_paths:
        ...
        if target is None:
            target = dataset.pixel_array
        else:
            target += dataset.pixel_array
    dataset.PixelData = target
    targetFile = "/data/qifan/projects/FastDoseWorkplace/TCIASelect/057/dose.dcm"
    dataset.save_as(targetFile)


def examineDose_new():
    """The interpretation above was not correct"""
    patientFolder = "/data/qifan/projects/FastDoseWorkplace/TCIASelect/057"
...
def dcm2nii():
    ...
    DataArray = DataArray[:, 110: -190, 150: -150]
...
def generateFullMask():
    ...
    fullArray[:, 110: -190, 150: -150] = maskArray
...
def preMaskCreation():
    ...
    print(names, sliceThickness)
    exclude = ["CTV70", "CTV56", "CTV63", "GTV", "BRAIN_STEM"]
    ...
    if False:
        # In the original annotation, skin the skin mask is incomplete. So here 
        # we are to complete it.
        mask_skin = np.zeros_like(mask_skin)
        idx_1 = int(35 * 512 / 200)
        idx_2 = int(135 * 512 / 200)
        mask_skin[2:, idx_1:idx_2, :] = 1
...
def PTVMerge():
    ...
    PTV_merge = np.logical_or(PTV3, PTV_merge)
...
def StructInfoGen():
    ...
    if True:
        ...
        expFolder = "/data/qifan/projects/FastDoseWorkplace/TCIASelect/057/FastDose"
...
def drawDVH_ref():
    ...
    ax.set_title("Reference dose for patient 009")
