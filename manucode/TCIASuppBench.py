...
resultFolder = "/data/qifan/projects/FastDoseWorkplace/TCIASupp"
figureFolder = "/data/qifan/projects/AAPM2024/manufigures"

patients = [2, 3, 9, 13, 70, 125, 132, 190]
patients = ["{:03d}".format(a) for a in patients]
exclude = ["SKIN", "PTVSeg0", "PTVSeg1", "PTVSeg2", "PTVSeg3", "PTVMerge"]
Converge = {"ORAL_CAVITY": ["oralcavity", "ORAL_CAVITY", "OralCavity"],
...
def StructsInit():
    ...
    for patient in patients:
        ...
        structuresLocal = [b for a in os.listdir(MaskFolder) if (b:=a.split(".")[0]) not in exclude]
        for a in structuresLocal:
            StructureSet.add(a)
    StructureList_copy = []
    for name in StructureSet:
    ...
    # move PTVs forward
    PTVList = ["PTV70", "PTV56", "leftptv56"]
    StructureList_copy.sort()
    StructureList_copy = [a for a in StructureList_copy if a not in PTVList]
    StructureList = PTVList + StructureList_copy
...
def DVH_plot():
    ...
    figureFile = os.path.join(figureFolder, "FastDoseTCIASupp.png")
    plt.savefig(figureFile)
    figureFile = os.path.join(figureFolder, "FastDoseTCIASupp.eps")
    plt.savefig(figureFile)
    plt.close(fig)
    plt.clf()
...
def SagittalPlot():
    """
    This function plots the sagittal slices of all patients
    """
    for patient in patients:
        patientFolder = os.path.join(resultFolder, patient)
        dimensionFile = os.path.join(patientFolder,
        ...
        PatientSagittalFolder = os.path.join(figureFolder, "DoseWashSample",
            "Sagittal{}".format(patient))
        if not os.path.isdir(PatientSagittalFolder):
            os.mkdir(PatientSagittalFolder)
        for i in range(dimension[2]):
            densitySlice = densityArray[:, :, i]
            doseSlice = DoseExp[:, :, i]
            if np.sum(doseSlice) == 0:
                continue
            MasksSlice = [(a, b[:, :, i]) for a, b in maskDict.items()]
            BodySlice = BodyMask[:, :, i]
            file = os.path.join(PatientSagittalFolder, "{:03d}.png".format(i))
            DrawSlice(densitySlice, doseSlice, MasksSlice, BodySlice, 80, 80, file)
            print(file)
