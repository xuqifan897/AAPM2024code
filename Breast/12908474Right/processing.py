...
def checkDose():
    ...
    CTFolder = os.path.join(patientFolder, "dicom")
    dicomDoseFile = os.path.join(patientFolder, "RTDose.dcm")
    dose = examineDose(dicomDoseFile, CTFolder)
    
    CTFiles = os.listdir(CTFolder)
    CTData = []
    HUmin = -1000
    for file in CTFiles:
        file = os.path.join(CTFolder, file)
    ...
    imageFolder = os.path.join(patientFolder, "doseView")
    if not os.path.isdir(imageFolder):
        os.mkdir(imageFolder)
    doseMax = np.max(dose)
    for i in range(len(CTData)):
        plt.imshow(CTData[i][1], cmap="gray", vmin=0, vmax=2000)
        doseSlice = dose[i, :, :]
