...
if [[ ${patientName}=="HGJ_001" && ${groupName}=="our_model" ]]; then
    for segment in 1 2 3 4; do
        outputFolder="${planFolder}/${groupName}_doseMat${segment}"
        ...
        logFile="${planFolder}/${groupName}_dosecalc${segment}.log"
        OMP_NUM_THREADS=64 ${exec} \
            ...
            --beamlist "${planFolder}/beamlist${segment}.txt" \
            --mode 0 \
            --deviceIdx 0 \
